...
export async function loadSessionMarkdown(filename) {
  // Use import.meta.glob with an alias and raw-query imports.
  // The @ alias typically points to the src/ directory.
  const markdownModules = import.meta.glob('@/assets/sessions/session-*.md', {
    query: '?raw',
    import: 'default'
  });
...
export async function loadSessionTranscript(filename) {
  const transcriptModules = import.meta.glob('@/assets/sessions/transcripts/*.txt', {
    query: '?raw',
    import: 'default'
  });
